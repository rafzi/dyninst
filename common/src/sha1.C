...
unsigned char finalcount[8];
    ...
    SHA1Update(context, (unsigned char *)const_cast< char *>("\200"), 1);
    while ((context->count[0] & 504) != 448) {
        SHA1Update(context, (unsigned char *)const_cast<char *>("\0"), 1);
    }
...
{
    static char result[SHA1_STRING_LEN] = {0};
    ...
    SHA1Init(&ctx);
    while (!feof(fd)) {
        unsigned long len = fread((char *)buf, sizeof(char), sizeof(buf), fd);
	if (ferror(fd)) {
	    fclose(fd);
	    return NULL;
	}
    ...
    for (unsigned int i = 0; i < SHA1_DIGEST_LEN; ++i)
        sprintf(&result_ptr[i*2], "%02x", buf[i]);
